...
可能是別的程式已經註冊了 **Win + Shift + C**(Windows 同一組全域快捷鍵只能有一個程式持有，而且查不出是誰)。FlashGrab 不會靜默失敗：

* 啟動時**自動改用備用鍵**，依序為 `Win + Shift + X`、`Ctrl + Alt + C`、`Ctrl + Alt + Shift + C`(只在這次執行生效，不寫進設定檔)，並跳出置頂對話框告訴你現在用哪一組。不靠 toast 通知，因為勿擾模式會把它吞掉。
* 全部組合都被佔用時，對話框會明說，托盤提示文字改為「快捷鍵未啟用」，托盤右鍵選單最上方多一列紅色 **⚠**。
* 想自己指定：托盤圖示 → **設定 → 取字快捷鍵**(勾修飾鍵＋選一個主鍵：A–Z、0–9、F1–F12，至少要一個修飾鍵)。按**儲存**時會實際註冊一次，被佔用就不給存。設定寫在 `settings.json` 的 `"Hotkey": "Ctrl+Alt+C"`；沒有這個欄位代表預設值。
